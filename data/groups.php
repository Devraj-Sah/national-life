...
class Groups {
  ...
  function getByParentId($parentId)
  {
    ...
    $sql = "SElECT * FROM groups WHERE parentId = '$parentId' AND hide = 'No' ORDER BY weight ASC, id DESC";
    $result = $conn->exec($sql);

    return $result;
  }
  /*******Edited by Madhu for Notice display**********/
  function getByParentIdNotice($parentId)
  {
    global $conn;

    $parentId = cleanQuery($parentId);

    $sql = "SElECT * FROM groups WHERE parentId = '$parentId' AND hide = 'No' ORDER BY id DESC";
    $result = $conn->exec($sql);

    return $result;
  }

    function getByParentIdAll($parentId)
  {
    global $conn;
...
///************Madhu Modified****************************/
  function getByParentIdWithLimitwithNo($parentId, $limit)
  {
    global $conn;


    $sql = "SElECT * FROM groups WHERE parentId = '$parentId' AND hide='No' ORDER BY weight DESC, onDate DESC, id DESC LIMIT $limit";
    $result = $conn->exec($sql);
  ...
  function getByParentIdWithLimitAsc($parentId, $limit)
  {
    ...
    $sql = "SElECT * FROM groups WHERE parentId = '$parentId' ORDER BY weight , onDate , id  LIMIT $limit";
    $result = $conn->exec($sql);
  ...
  function writeBreadCrumb($id, $currentPage)
  {
    ...
    //if (count($list) > 1)
    {
      echo "<li class='active'>" . $currentPage . "</li>";
      echo '</ol>';
    }
  }
  ...
  function getBreadCrumb($id, &$list)
  {
    ...
    while ($row = $conn->fetchArray($result))
    {
      ...
      array_push($list, "<li><a  href='" . $row['urlname'] . "'>" . $row['name'] . "</a></li>");
